...
class Dashboard extends React.Component {
    ...
    releaseParking(parking) {
        ...
        fetch('http://localhost:5000/api/releaseParking', requestOptions)
            .then(res => res.json())
            .then(res => {
                console.log(res);
    ...
    bookParkingHandler(e) {
        ...
        fetch('http://localhost:5000/api/bookParking', requestOptions)
            .then(res => res.json())
            .then(res => {
                console.log(res);
                let parkingSpace = this.state.parkingSpaceList;
                    ...
                    <CardColumns className="parking-space-list">
                        {
                            parkingSpaceList && parkingSpaceList.map((v, i, a) => (
                                filterZone === "All" || v.zone_id === filterZone ? (
                                    <Card
                                        key={i}
                                        className={`parking-item ${v.is_available ? "available" : "not-available"} ${canBookParking ? "show-cursor" : ""}`}
                                        title={canBookParking ? v.is_available ? "Book Parking" : "Release Parking" : v.is_available ? "Available" : "Booked"}
                                        onClick={canBookParking ? v.is_available ? this.bookParking.bind(this, v) : this.releaseParking.bind(this, v) : null}
                                    >
                                        <Card.Body>
                                            <Card.Text>
                                                {v.title}
